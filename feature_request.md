...
- [x] PWA + Admin: Nach dem Bearbeiten eines Assets wurden neu hinzugefügte Fotos in der Asset-Detail-Seite nicht angezeigt – Ursache war `next/image` das Supabase-Storage-URLs ohne Domain-Konfiguration blockiert; in `asset/[qr]/page.tsx` und `admin/assets/[id]/page.tsx` durch natives `<img>`-Tag ersetzt, Fotos als klickbare Links zum Original geöffnet
- [x] WebView / PWA: Historie-Einträge zeigen jetzt immer Ersteller-Name (username) und Uhrzeit an; Klick auf einen Eintrag öffnet ein Detail-Modal (WebView: Dialog-Overlay; PWA: BottomSheet) mit allen Feldern (Ersteller, Datum & Uhrzeit, km-Stand, Tankstatus, Notiz, Fotos, Link zu Asset-Details)
- [x] WebView: Aktivitätslog auf Asset-Detailseite verwendet eigene `ActivityLog`-Komponente (statt PWA `LogHistoryList`/BottomSheet) – zeigt Einträge als klickbare Zeilen mit Aktion, Ersteller, Datum+Uhrzeit; Klick öffnet zentriertes Modal-Overlay mit Detailansicht (Von, Datum & Uhrzeit, km-Stand, Tankstatus, Notiz, Fotos)
